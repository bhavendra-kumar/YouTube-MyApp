...
export function UserProvider({ children }: { children: React.ReactNode }) {
  ...
  const logout = async () => {
    ...
    try {
      ...
      await axiosClient.post("/auth/logout", undefined, { skipAuthRefresh: true } as any);
    } catch {
      // ignore
    }
  ...
  const getGoogleRedirectProfile = async (): Promise<SocialProfile | null> => {
    ...
    try {
      ...
      // Bubble up Firebase auth errors with code so the login page can show a useful message.
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const backendUser = res.data?.user || res.data?.result;
        if (typeof token === "string" && token.length > 0 && backendUser) {
