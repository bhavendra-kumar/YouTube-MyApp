...
const baseURL = getPublicBackendApiUrl() || undefined;
...
async function refreshAccessToken(client: AxiosInstance): Promise<string | null> {
  ...
  refreshInFlight = (async () => {
    try {
      const res = await client.post("/auth/refresh", undefined, {
  ...
  client.interceptors.response.use(
    ...
    async (error: AxiosError) => {
      ...
      const isAuthRoute =
        ...
        // Backward compatibility with any remaining legacy callers.
