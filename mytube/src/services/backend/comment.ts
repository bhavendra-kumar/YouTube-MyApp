...
import { validateCommentText } from "@/lib/backend-utils/moderation";
import { translateText } from "@/services/backend/translate";
...
export const postcomment = async (reqArgs: any = {}) => {
  const req = reqArgs;
  ...
  try {
    ...
    return { data: { comment: true, data: created }, status: 200, cookies: resContext.cookies };
  } catch (err) {
...
export const getallcomment = async (reqArgs: any = {}) => {
  const req = reqArgs;
  const resContext = { cookies: [] };
  const res = {
    cookie: (n,v,o) => resContext.cookies.push({n,v,o,clear:false}),
...
export const deletecomment = async (reqArgs: any = {}) => {
  const req = reqArgs;
  const resContext = { cookies: [] };
  const res = {
    cookie: (n,v,o) => resContext.cookies.push({n,v,o,clear:false}),
  ...
  try {
    ...
    if (!existing.isDeleted) {
      await Comment.findByIdAndUpdate(_id, { $set: { isDeleted: true } }, { returnDocument: "before" }).lean();
      const io = req.app.get("io");
    ...
    return { data: { comment: true }, status: 200, cookies: resContext.cookies };
  } catch (err) {
    if (err instanceof AppError) throw err;
...
export const editcomment = async (reqArgs: any = {}) => {
  const req = reqArgs;
  const resContext = { cookies: [] };
  const res = {
    cookie: (n,v,o) => resContext.cookies.push({n,v,o,clear:false}),
  ...
  try {
    ...
    return { data: updatecomment, status: 200, cookies: resContext.cookies };
  } catch (err) {
    if (err instanceof AppError) throw err;
...
export const likeComment = async (reqArgs: any = {}) => {
  const req = reqArgs;
  const resContext = { cookies: [] };
  const res = {
    cookie: (n,v,o) => resContext.cookies.push({n,v,o,clear:false}),
...
export const dislikeComment = async (reqArgs: any = {}) => {
  const req = reqArgs;
  const resContext = { cookies: [] };
  const res = {
    cookie: (n,v,o) => resContext.cookies.push({n,v,o,clear:false}),
...
export const translateComment = async (reqArgs: any = {}) => {
  const req = reqArgs;
  const resContext = { cookies: [] };
  const res = {
    cookie: (n,v,o) => resContext.cookies.push({n,v,o,clear:false}),
