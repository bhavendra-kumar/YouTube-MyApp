...
import { getRazorpayClient } from "@/services/backend/razorpay";
import { sendInvoiceEmail } from "@/services/backend/emailService";
...
export const createOrder = async (reqArgs: any = {}) => {
  const req = reqArgs;
...
export const verifyPayment = async (reqArgs: any = {}) => {
  const req = reqArgs;
  const resContext = { cookies: [] };
  const res = {
    cookie: (n,v,o) => resContext.cookies.push({n,v,o,clear:false}),
