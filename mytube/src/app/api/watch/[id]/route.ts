...
import * as service from '@/services/backend/watchlater';
...
export async function GET(req: NextRequest, props: any) {
  try {
    ...
    const result = await service.getallwatchlater(reqArgs);
...
export async function POST(req: NextRequest, props: any) {
  try {
    ...
    const result = await service.handlewatchlater(reqArgs);
    const response = NextResponse.json(result.data, { status: result.status });
