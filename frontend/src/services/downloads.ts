...
export async function requestVideoDownload(videoId: string): Promise<DownloadResponse> {
  ...
  const res = await axiosClient.post<DownloadResponse>("/api/download", { videoId: id });
...
// Best-effort browser download.
// - First tries fetch->blob (works cross-origin only if CORS allows it)
// - Falls back to opening the URL directly.
