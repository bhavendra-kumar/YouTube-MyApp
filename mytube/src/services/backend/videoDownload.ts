...
export const downloadVideo = async (reqArgs: any = {}) => {
  const req = reqArgs;
  ...
  if (fs.existsSync(finalFullPath)) {
    ...
    return { data: { message: "Video downloaded successfully", path: publicPath, downloadUrl: publicPath }, status: 200, cookies: resContext.cookies };
  }
